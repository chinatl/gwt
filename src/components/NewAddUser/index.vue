...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["user_info", "org_role_list", "is_admin"])
  },
  methods: {
    init() {
      if (this.orgId) {
        ...
        this.$post(
          ...
          .then(res => {
            ...
            var arr = res.data.sysOrgs.map(res => {
              if (this.orgId == res.orgId) {
                res.nodeType = "ORG_GROUP";
              } else {
                res.nodeType = "";
              }
              res.id = res.orgId;
              res.pId = res.orgParentId;
              res.name = res.orgName;
              return res;
            });
            this.part_tree = generate_tree(arr);
            this.$post(
              "gwt/system/sysUser/loadOrgUser",
              {
                orgId: "",
                orgIds: res.data.sysOrgs.map(res => res.orgId).join(","),
                fromCommon: "Y",
                fromPerson: "Y",
                realName: ""
              },
              ...
              .then(response => {
                ...
                this.part_user_list = response.data.userOrgs;
