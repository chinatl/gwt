    ...
    <div class="notice-desc" v-loading='desc_loading'>
        ...
        <div class="article-main">
            <div class="artive-address">
                <p v-if='data.endTime'>签收截止时间：<span>{{data.endTime}}</span></p>
            </div>
...
export default {
  ...
  methods: {
    init_reve(pageSize, currentPage) {
      ...
      this.$post(
        `gwt/website/tbWebsite/orgReceiveList?${qs.stringify({
          currentPage,
          pageSize
        })}`,
        {
    ...
    init(pageSize, currentPage) {
      ...
      var data = {
       ...
       if (this.dataType === 5) {
        url = `gwt/website/tbWebsite/smsWebsiteFailedList`;
        data.dataType = 4;
      } else {
        url = `gwt/website/tbWebsite/itemList`;
      }
      this.$post(
        `${url}?${qs.stringify({
          currentPage,
    ...
    re_tip_user() {
      this.$post(
        "gwt/website/tbWebsite/smsRemind",
        {
          noticeId: this.message_data.NOTICE_ID
        },
        "json"
    ...
    //查看类型数量
    get_type_nmuber() {
        console.log(this.message_data.NOTICE_ID)
      this.$post(
        "gwt/website/tbWebsite/tabCounts",
        {
          noticeId: this.message_data.NOTICE_ID,
          forwardId: this.forwardId,
          attaUploadNode: 2
    ...
    init_file(noticeId) {
      this.$post(
        "gwt/system/tbWebsiteAttachment/attachmentList",
        {
          noticeId,
          attaUploadNode: 1
        },
